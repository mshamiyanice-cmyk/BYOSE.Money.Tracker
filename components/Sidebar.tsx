...
import { X, LogOut, UserCircle, LayoutDashboard, ListTodo } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ userEmail, inflows, overdrafts, isAdmin, profile, isOpen, onClose }) => {
  const links = [
    ...
    { to: '/ledger', icon: 'fa-list-ul', label: 'Master Ledger' },

    { to: '/calendar', icon: 'fa-calendar-alt', label: 'Calendar' },
    { to: '/inflows', icon: 'fa-arrow-trend-up', label: 'Inflows' },
    { to: '/outflows', icon: 'fa-arrow-trend-down', label: 'Outflows' },
    { to: '/overdrafts', icon: 'fa-receipt', label: 'Overdrafts' },
    { to: '/tracker', icon: 'fa-route', label: 'Flow Tracker' },
