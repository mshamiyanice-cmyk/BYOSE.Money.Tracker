...
const InflowManager: React.FC<InflowManagerProps> = ({ inflows, onAdd, onUpdate, onDelete, onRepay, isAdmin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.source || isNaN(amountNum)) return;
    ...
    setFormData({ source: '', product: 'Service Revenue', amount: '', date: new Date().toISOString().split('T')[0], description: '' });
    setShowForm(false);
    setEditingId(null);
  };
    ...
    <div className="space-y-6 pb-20 lg:pb-0">
      ...
      {isAdmin && showForm && (
        <form onSubmit={handleSubmit} className="bg-white p-4 md:p-8 rounded-[24px] shadow-2xl border border-slate-100 animate-in fade-in slide-in-from-top-4 duration-500 overflow-visible relative z-30">
          ...
          <Button type="submit" className="mt-10 w-full h-14 text-lg font-black rounded-2xl shadow-xl shadow-[#165b4c]/20 hover:shadow-[#165b4c]/30 transition-all active:scale-95">
