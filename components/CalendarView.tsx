          ...
          {days.map((day, idx) => {
            ...
            return (
              <div 
                ...
                className={`p-2 border border-slate-100 min-h-[140px] relative transition-all cursor-pointer group hover:z-10 hover:shadow-xl hover:border-[#165b4c]/30 ${
                  hasActivity ? 'bg-white' : 'bg-slate-50/10'
                }`}
              >
                <div className="flex justify-between items-center mb-2">
                  <span className={`text-xs font-bold w-6 h-6 flex items-center justify-center rounded-full transition-colors ${
                    isToday ? 'bg-[#165b4c] text-white' : 'text-slate-400 group-hover:text-slate-800'
                  }`}>
                    {day}
                  </span>
                  {hasActivity && (
