...
interface OverdraftManagerProps {
  ...
  onAdd: (overdraft: Overdraft) => Promise<void>;
  onUpdate: (overdraft: Overdraft) => Promise<void>;
  onSettle: (overdraftId: string, inflowId: string) => Promise<void>;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const handleSettle = async (e: React.FormEvent) => {
      ...
      if (showSettleModal && selectedInflowId) {
        setIsSettling(true);
        try {
          await onSettle(showSettleModal, selectedInflowId);
    ...
    const handleDelete = async (id: string) => {
      if (!confirm("Are you sure you want to delete this overdraft? If it was settled, the payment will be refunded.")) return;
      setDeletingId(id);
      try {
        await onDelete(id);
      } catch (error) {
        ...
        {isAdmin && showForm && (
          <form onSubmit={handleSubmit} className="bg-white p-6 md:p-10 rounded-[32px] shadow-2xl border border-rose-100 animate-in fade-in slide-in-from-top-4 duration-500 overflow-hidden relative z-30">
            ...
            <Button
              ...
              className="mt-12 w-full h-16 text-xl font-black rounded-3xl shadow-2xl shadow-rose-200 transition-all active:scale-[0.98] bg-rose-600 hover:bg-rose-700 flex items-center justify-center gap-2"
            >
              {isSubmitting && <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />}
              {isSubmitting ? 'Processing Audit...' : 'Authorize External Liability'}
            </Button>
          </form>
        )}
              ...
              <form onSubmit={handleSettle} className="space-y-6 md:space-y-8">
                ...
                <div className="flex flex-col gap-4">
                  <Button
                    type="submit"
                    disabled={isSettling}
                    className="h-14 md:h-16 text-lg md:text-xl font-black rounded-3xl shadow-2xl shadow-[#165b4c]/20 flex items-center justify-center gap-2"
                  >
                    {isSettling && <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />}
                    {isSettling ? 'Processing...' : 'Authorize Settlement'}
                  </Button>
                      ...
                      {isAdmin && (
                        <td className="px-10 py-6 text-right whitespace-nowrap">
                          <div className="flex justify-end items-center gap-3">
                            ...
                            <button onClick={() => handleDelete(od.id)} className="w-10 h-10 flex items-center justify-center text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded-2xl transition-all">
                              {deletingId === od.id ? <div className="w-4 h-4 border-2 border-rose-600 border-t-transparent rounded-full animate-spin" /> : <Trash2 size={18} />}
                            </button>
                          </div>
                        </td>
                      )}
